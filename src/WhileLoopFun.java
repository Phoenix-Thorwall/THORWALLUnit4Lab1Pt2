public class WhileLoopFun {
    ...
    {
        String num = "" + number;
        int lenCount = num.length();
        while (lenCount != 0)
        {
            System.out.println(num.substring(lenCount - 1, lenCount));
            lenCount--;
        }
    ...
    public int countLetter(String word, String letter) {
        int wordle = word.length();
        int letTrack = 1;
        int count = 0;
        while (letTrack <= wordle)
        {
            if (word.substring(letTrack - 1, letTrack).equals(letter))
            {
                count ++;
            }
            letTrack ++;
